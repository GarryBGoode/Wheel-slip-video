...
def Round_Corner_Param2(radius,target_mob : VMobject, i):
    def get_mob_point(t):
        j = int(t) % target_mob.get_num_curves()
        a = t % 1
        return target_mob.get_nth_curve_function(j)(a)
    func1 = get_mob_point
    diff_func_1 = lambda t: (func1(t)-func1(t-1e-6))
    func2 = get_mob_point
    diff_func_2 = lambda t: (func2(t+1e-6)-func2(t))
    ...
    sol = root(rad_cost_func,np.asarray([i+0.99,i+1.01]),method='lm')
    k = sol['x']
...
def Round_Corners(mob:VMobject,radius=0.2, arc_components=5):
    i=0
    j=0
    while i < mob.get_num_curves() and j<1e5:
        ...
        if abs(angle_test)>1E-6:
            params, k = Round_Corner_Param2(radius,mob,i)
            # cut_curve_points_1 = partial_bezier_points(curve_1, 0, k[0])
            # cut_curve_points_2 = partial_bezier_points(curve_2, k[1], 1)
            loc_mob_1 = mob.copy().pointwise_become_partial(mob,0, k[0]/mob.get_num_curves())
            loc_mob_2 = mob.copy().pointwise_become_partial(mob,k[1] / mob.get_num_curves(), 1)
            loc_arc = Arc(**params,num_components=arc_components)
            ...
            mob.points = loc_mob_1.points.copy()
            mob.points = np.append(mob.points, loc_arc.points, 0)
            mob.points = np.append(mob.points, loc_mob_2.points, 0)
