...
class Path_Offset_Mobject(VMobject):
    def __init__(self,
        ...
        self.update_t_range(discontinuities,num_of_samples)
    ...
    def update_t_range(self,discontinuities=[],num_of_samples=100):
        self.t_range = np.linspace(0, 1 - 1e-6, num_of_samples)
        if discontinuities != []:
            t_disc = np.sort(
            ...
            if self.target.is_closed():
                ...
                t_disc = t_disc[0 <= t_disc <= 1]

            for disc_val in discontinuities:
                if disc_val in self.t_range:
                    self.t_range = self.t_range[self.t_range != disc_val]

            self.t_range = np.unique(np.sort(np.concatenate((self.t_range, t_disc))))
    ...
    def generate_offset_paths(self,gen_ofs_point=True, gen_ref_curve=True):
        ...
        points = self.ref_curve.points + ofs_vectors

        if self.target.is_closed():
            if any(points[0,:] != points[-1,:]):
                p1 = points[-1,:]
                p2 = (points[-1, :] * 2 + points[0, :]) / 3
                p3 = (points[-1, :] + 2 * points[0, :]) / 3
                p4 = points[0, :]
                points = np.append(points,[p1,p2,p3,p4],0)
...
class Curve_Warp(VMobject):
    def __init__(self,warp_source:Mobject,warp_curve:Mobject,anchor_point=0.5, **kwargs):
    ...
    def generate_points(self):
        if len(self.warp_source.points)>0:
...
class Curve_Warp_Group(Curve_Warp):
    def __init__(self,warp_source:Mobject,warp_curve:Mobject,anchor_point=0.5, **kwargs):
        super().__init__(warp_source,warp_curve,anchor_point=anchor_point, **kwargs)
        for mob in warp_source.submobjects:
            self.add(Curve_Warp(mob,warp_curve,anchor_point,**kwargs))